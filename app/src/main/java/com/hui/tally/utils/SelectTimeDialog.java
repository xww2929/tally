...
public class SelectTimeDialog extends Dialog implements View.OnClickListener {
    ...
    public void onClick(View view) {
        if (view.getId()==R.id.dialog_time_btn_cancel){
            cancel();
        } else if (view.getId()==R.id.dialog_time_btn_ensure) {
            int year=datePicker.getYear();//获取选择的年份
            int month=datePicker.getMonth()+1;//获取选择的月份
            //int month=datePicker.getMonth();//获取选择的月份
            int dayOfMonth=datePicker.getDayOfMonth();//获取选择的日
    ...
    private void hideDatePickerHeader(){
        ...
        //5.0+
        int headerId=getContext().getResources().getIdentifier("day picker_selector_layout","id","android");
